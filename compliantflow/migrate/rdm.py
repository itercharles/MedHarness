...
def generate_migration_markdown(completeness_report: Dict) -> str:
    """Render a post-migration completeness report as a Markdown document.

    Args:
        completeness_report: The dict returned by
            ``CompliantFlowCore.validate_migration_report()``.
    """
    from datetime import datetime, timezone

    summary = completeness_report.get("summary", {})
    clean = completeness_report.get("clean", [])
    gaps = completeness_report.get("gaps", [])
    skipped = completeness_report.get("skipped", [])

    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    total = summary.get("total_created", 0)
    n_clean = summary.get("clean", 0)
    n_gaps = summary.get("gaps", 0)
    n_skipped = summary.get("skipped", 0)

    lines: List[str] = [
        "# RDM Migration Completeness Report",
        "",
        f"**Generated:** {ts}  ",
        f"**Migrated items:** {total} created, {n_clean} clean, {n_gaps} with gaps, {n_skipped} skipped",
        "",
    ]

    # Overall status
    if n_gaps == 0 and n_skipped == 0:
        lines += ["**Status: ✓ Migration complete — no compliance gaps.**", ""]
    else:
        lines += [
            f"**Status: ⚠ Review required — {n_gaps} item(s) have compliance gaps, "
            f"{n_skipped} item(s) could not be migrated.**",
            "",
        ]

    # Clean items
    lines += [f"## ✓ Clean Items ({n_clean})", ""]
    if clean:
        lines += ["| ID | Type | Title |", "|---|---|---|"]
        for item in clean:
            title = str(item.get("title", "")).replace("|", "\\|")
            lines.append(f"| {item['cf_id']} | {item['type']} | {title} |")
    else:
        lines += ["*No clean items.*"]
    lines += [""]

    # Gap items
    lines += [f"## ⚠ Items Requiring Review ({n_gaps})", ""]
    if gaps:
        for item in gaps:
            lines += [f"### {item['cf_id']} (from `{item['rdm_id']}`)", ""]
            mw = item.get("migration_warnings", [])
            errs = item.get("validation_errors", [])
        ...
        lines += ["*No compliance gaps.*", ""]

    # Skipped items
    lines += [f"## ✗ Skipped Items ({n_skipped})", ""]
    if skipped:
        lines += ["| RDM ID | Reason |", "|---|---|"]
        for item in skipped:
            reason = str(item.get("reason", "")).replace("|", "\\|")
            lines.append(f"| {item.get('rdm_id', '?')} | {reason} |")
        lines += [
            "",
    ...
    lines += [
        "---",
        "*Generated by `compliantflow migrate rdm --validate`*",
    ]
