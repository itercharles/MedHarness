...
def _traceability_html(matrix: Dict[str, Any]) -> str:
    ...
    col_headers += "<th>Status</th>"

    body_rows = []
    for r in rows:
        if r.get("is_orphan"):
            css = "orphan"
            status = f'<span class="fail">ORPHAN ({r.get("orphan_type", "")})</span>'
        elif not r.get("is_complete"):
            css = "incomplete"
            status = '<span class="fail">INCOMPLETE</span>'
        else:
            css = ""
            status = '<span class="pass">COMPLETE</span>'
