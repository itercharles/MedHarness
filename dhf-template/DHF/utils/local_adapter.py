...
class LocalDHFAdapter:
    ...
    def get_project_config(self):
        from compliantflow.domain.schema import (
            ProjectSchema, ItemTypeSchema, GlobalLifecycle, LifecycleStateInfo, FieldSchema
        )
        item_types = []
        for dt in self._config.doc_types:
            ...
            for prop in (dt.properties or []):
                if isinstance(prop, str):
                    # Simple field like "id" — no constraints
                    fields.append(FieldSchema(name=prop))
                elif isinstance(prop, dict) and "name" in prop:
                    fields.append(FieldSchema(
                        name=prop["name"],
                        format=prop.get("format", "short_text"),
                        label=prop.get("label", ""),
                        required=bool(prop.get("required", False)),
                        options=prop.get("options") or prop.get("values") or [],
                        default=prop.get("default"),
                        target_types=prop.get("target_types") or [],
                    ))
